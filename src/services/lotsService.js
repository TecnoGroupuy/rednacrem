...
export const listLotsAsync = async () => {
  if (hasApiConfigured()) {
    ...
    return (response?.items || []).map((item) => ({
      ...
      ventas: Number(item.ventas ?? item.total_ventas ?? 0) || 0,
      rechazos: Number(item.rechazos ?? item.total_rechazos ?? 0) || 0,
      no_contesta: Number(item.no_contesta ?? item.total_no_contesta ?? 0) || 0,
      dato_erroneo: Number(item.dato_erroneo ?? item.total_dato_erroneo ?? 0) || 0,
      nuevos: Number(item.nuevos ?? item.total_nuevos ?? 0) || 0,
      count: Number(item.total_contactos ?? item.cantidad_contactos ?? item.cantidad ?? item.count ?? 0) || 0,
